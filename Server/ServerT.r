...
#setwd('E:/OneDrive/PhD/Project/Thesis/4_Pipeline/Pipeline/Server/PXD012039/NormalisedData')
norm <- args[1]
dataSet <- args[2]
...
for (k in count){
  ...
  ttest <- tryCatch(t.test(as.numeric(compN[k,][index1]), 
                           na.rm = TRUE, as.numeric(compN[k,][index2]), 
